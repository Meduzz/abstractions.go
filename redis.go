...
func CreateRedisConfig(conn *redis.Client, prefix string) *vendor.RedisConfig {
	return vendor.NewRedisConfig(conn, prefix)
}

// RedisCaching - create a new caching module with the provided config and codec.
func RedisCaching[T any](config *vendor.RedisConfig, codec lib.Codec[T], ttl time.Duration, name string) lib.CacheAbstraction[T] {
	return caching.NewCaching[T](config, codec, ttl, name)
}

// RedisCSRF - create a new CSRF module with the provided config.
func RedisCSRF(config *vendor.RedisConfig, ttl time.Duration, name string) lib.CSRFAbstraction {
	return csrf.NewCSRFAbstraction(config, ttl, name)
}

// RedisEventing - create a new eventing module.
func RedisEventing[T any](topic string, codec lib.Codec[T], config *vendor.RedisConfig) lib.EventingAbstraction[T] {
	return eventing.NewEventing[T](topic, codec, config)
}

// RedisLog - create a new redis based work log.
func RedisLog[T any](config *vendor.RedisConfig, codec lib.Codec[T], name string) lib.LogAbstraction[T] {
